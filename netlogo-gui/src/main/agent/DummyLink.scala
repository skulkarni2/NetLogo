...
class DummyLink(coreWorld: CoreWorld, _end1: AnyRef, _end2: AnyRef, breed: AgentSet)
extends Link(coreWorld, DummyLink.toTurtle(_end1),
             DummyLink.toTurtle(_end2),
             coreWorld.getLinkVariablesArraySize(breed)) {
  variables(Link.VAR_BREED) = breed
  variables(Link.VAR_END1) = end1
  variables(Link.VAR_END2) = end2
  override def toString =
    coreWorld.getLinkBreedSingular(getBreed).toLowerCase + " " +
