...
class World3D extends World with org.nlogo.api.World3D with CompilationManagement {
  ...
  override def setDimensionVariable(variableName: String, value: Int, d: WorldDimensions): WorldDimensions = {
    ...
    if (variableName.equalsIgnoreCase("MIN-PZCOR")) {
      wd.copyThreeD(minPzcor = value)
    } else if (variableName.equalsIgnoreCase("MAX-PZCOR")) {
      wd.copyThreeD(maxPzcor = value)
    } else if (variableName.equalsIgnoreCase("WORLD-DEPTH")) {
      val minPzcor = growMin(wd.minPzcor, wd.maxPzcor, value, wd.minPzcor)
      val maxPzcor = growMax(wd.minPzcor, wd.maxPzcor, value, wd.maxPzcor)
      wd.copyThreeD(maxPzcor = maxPzcor, minPzcor = minPzcor)
    } else {
